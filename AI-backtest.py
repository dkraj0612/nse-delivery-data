...
BATCH_SIZE = 8
SLEEP_BETWEEN_BATCHES_MINUTES = 5

os.makedirs(OUTPUT_DIR, exist_ok=True)

# [All functions: load_progress, save_progress, generate_analysis, get_all_dates remain same as previous]

# ========================= DASHBOARD GENERATOR =========================
def generate_dashboard():
    print("📊 Compiling results and generating Institutional Dashboard...")
    
    # 1. Read all generated JSON files from the reports directory
    compiled_data = {}
    if os.path.exists(OUTPUT_DIR):
        for filename in os.listdir(OUTPUT_DIR):
            if filename.endswith(".json"):
                # Convert filename like '2023_12.json' back to '2023-12'
    ...
    if not compiled_data:
        print("  ⚠️ No data found to generate dashboard.")
        return

    # 2. Convert to JavaScript-friendly JSON string
    json_data_str = json.dumps(compiled_data)

    # 3. HTML Template (V3 Dashboard)
    html_template = """<!DOCTYPE html>
<html lang="en" class="dark">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Institutional Quant Dashboard - v3</title>
...
<body class="flex h-screen overflow-hidden antialiased">
        ...
        <div class="flex-1 overflow-y-auto p-8 custom-scrollbar relative">
            ...
            <div class="max-w-[1600px] mx-auto space-y-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <div class="glass-card p-4 relative overflow-hidden"><p class="text-xs text-gray-400 font-medium mb-1">Alpha (Ann.)</p><h3 class="text-2xl font-bold num-font tracking-tight text-white" id="kpi-alpha">--</h3><p class="text-[10px] mt-2 text-gray-500">Jensen's Measure</p></div>
                    <div class="glass-card p-4 relative overflow-hidden"><p class="text-xs text-gray-400 font-medium mb-1">Information Ratio</p><h3 class="text-2xl font-bold num-font tracking-tight text-white" id="kpi-ir">--</h3><p class="text-[10px] mt-2 text-gray-500">Active Risk Adj.</p></div>
                    <div class="glass-card p-4 relative overflow-hidden"><p class="text-xs text-gray-400 font-medium mb-1">Sharpe Ratio</p><h3 class="text-2xl font-bold num-font tracking-tight text-white" id="kpi-sharpe">--</h3><p class="text-[10px] mt-2 text-gray-500">Risk Free: 4.2%</p></div>
                    <div class="glass-card p-4 relative overflow-hidden"><p class="text-xs text-gray-400 font-medium mb-1">Max Drawdown</p><h3 class="text-2xl font-bold num-font tracking-tight text-neg" id="kpi-dd">--</h3><p class="text-[10px] mt-2 text-gray-500">Intra-month</p></div>
                    <div class="glass-card p-4 relative overflow-hidden"><p class="text-xs text-gray-400 font-medium mb-1">Portfolio Beta</p><h3 class="text-2xl font-bold num-font tracking-tight text-white" id="kpi-beta">--</h3><p class="text-[10px] mt-2 text-gray-500">vs. Benchmark</p></div>
                </div>
    ...
    <script>
        ...
        // Python will inject the actual compiled JSON here
        ...
        function init() {
            displayMonths = Object.keys(advancedData).sort((a,b) => b.localeCompare(a));
            ...
            displayMonths.forEach((m, i) => {
                ...
                // Add fallback to avoid undefined errors if metrics is missing in some JSON output
    ...
    print("✅ Successfully created 'dashboard.html'! Open this file in your browser to view.")

# ========================= MAIN =========================
if __name__ == "__main__":
    print("🚀 Starting FULLY AUTOMATIC SELF-RESUMING Backtest...\n")
    ...
    if not remaining:
        ...
        while remaining:
            ...
            print(f"🔄 Starting batch of {len(current_batch)} months...")
            
            results = {}
            newly_completed = []

            for i, date_str in enumerate(current_batch):
                print(f"  [{i+1}/{len(current_batch)}] {date_str}")
                analysis = generate_analysis(date_str)
                results[date_str] = analysis
                
                with open(f"{OUTPUT_DIR}/{date_str.replace('-', '_')}.json", "w", encoding="utf-8") as f:
                    json.dump(analysis, f, indent=2, ensure_ascii=False)
                
                newly_completed.append(date_str)
                time.sleep(35 + random.uniform(8, 15))
            ...
            if remaining:
                print(f"⏳ Batch done. Sleeping {SLEEP_BETWEEN_BATCHES_MINUTES} minutes...\n")
                time.sleep(SLEEP_BETWEEN_BATCHES_MINUTES * 60)
    
    print(f"\n🎉 BACKTEST FINISHED! Total Processed: {len(completed)}")
    
    # Generate the visual dashboard output when everything is finished
    if len(completed) > 0:
        generate_dashboard()
